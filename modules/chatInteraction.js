  ...
  try {
    client.on('messageCreate', async (message) => {
      try {
        ...
        if (cmd === 'help') return message.reply('Commands: `$help`, `$afk <reason>`, `$removeafk`, `$lf <game>`, `$lfon`, `$lfoff`');
        ...
        if (cmd === 'lfon') return message.reply('You are now opted in to LF$ DMs (default).');
        if (cmd === 'lfoff') return message.reply('You are now opted out of LF$ DMs (placeholder).');
    ...
    client.on('guildMemberAdd', async (member) => {
      try {
        ...
        if (welcome) await welcome.send(greetings[Math.floor(Math.random()*greetings.length)]);
      } catch (err) {
        console.error
