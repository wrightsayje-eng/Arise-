// 🧩 logging.js — v1.4.2 Pro (DeXVyBz Logging System)
...
export default async function setupLogging(client) {
  ...
  const log = async (type, target, reason, moderator = 'System', extra = '') => {
    try {
      ...
      console.error('[LOGGING] Error in log()', err);
    }
  };
  ...
  client.on('guildMemberRemove', async (member) => {
    try {
      ...
      console.error('[LOGGING] guildMemberRemove error', err);
    }
  });
  ...
  console.log('✅ DeXVyBz Logging system active (v1.4.2 Pro)');
}
