// 🎵 musicCommands.js v1.5 — Pro Music Commands + Queue + Buttons
import {
  joinVoiceChannel,
  getVoiceConnection,
  createAudioPlayer,
...
// ----- Helper: Convert Apple Music URL to YouTube fallback -----
...
export default async function setupMusicPro(client) {
  console.log('✅ Music Pro Module loaded');
  ...
  client.on('messageCreate', async (message) => {
    ...
    const getGuildObj = () => {
      if (!guildPlayers.has(message.guild.id)) {
        ...
        guildPlayers.set(message.guild.id, {
          player,
          queue: [],
          nowPlaying: null,
    ...
    async function updateNowPlayingMessage(gObj, msgOrInteraction) {
      const embed = new EmbedBuilder()
        ...
        .addFields({
          name: 'Queue',
          value: gObj.queue.length
            ? gObj.queue.map((t, i) => `${i + 1}. ${t}`).join('\n')
            : 'Empty',
        })
        .setColor('Purple');
      ...
      try {
        if (gObj.nowPlayingMessage) {
          await gObj.nowPlayingMessage.edit({ embeds: [embed], components: [buttons, selectMenu] });
        } else {
        ...
        console.error('[MUSIC] updateNowPlayingMessage error:', err);
      }
    }
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
      switch (interaction.customId) {
        ...
        case 'repeat':
          gObj.repeat = !gObj.repeat;
          await interaction.reply({ content: `🔁 Repeat ${gObj.repeat ? 'enabled' : 'disabled'}`, ephemeral: true });
          break;
      }
    }
